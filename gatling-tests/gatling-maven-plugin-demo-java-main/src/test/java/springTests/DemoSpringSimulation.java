...
public class DemoSpringSimulation extends Simulation {
    ...
    HttpProtocolBuilder httpProtocol = http
            .baseUrl("http://spring.local") // URL da sua API Spring Boot
    ...
    ScenarioBuilder scn = scenario("DemoSpringAPI Test")
            ...
            .exec(http("Get Entity By ID")
                    .get("/api/demo/#{entityId}")  // Usando o ID salvo na requisição GET
                    .check(status().is(200)))
            ...
            .exec(http("Update Entity")
                    .put("/api/demo/#{entityId}") // Aqui a ID é estática apenas como exemplo, mas você pode tornar dinâmico
            ...
            .exec(http("Delete Entity")
                    .delete("/api/demo/#{entityId}")  // ID de novo sendo estática, pode ser dinâmica se necessário
                    .check(status().is(200)));
